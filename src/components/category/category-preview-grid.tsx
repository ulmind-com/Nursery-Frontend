    ...
    <div className="grid grid-cols-2 gap-3 sm:gap-5 md:grid-cols-3 lg:gap-x-5 lg:gap-y-8">
      {previewItemsFor(slug).map((item, index) => (
        <article key={`${item.title}-${index}`} className="group min-w-0 overflow-hidden rounded-xl bg-card">
          <Link to="/product/$id" params={{ id: item.id }} className="relative block aspect-[1.08/1] overflow-hidden bg-muted">
          ...
          <div className="p-3 sm:p-4">
            <h3 className="line-clamp-1 text-base text-forest sm:text-xl"><Link to="/product/$id" params={{ id: item.id }}>{item.title}</Link></h3>
