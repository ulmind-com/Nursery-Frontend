...
import { CartActions, quickAddFromProduct } from "@/components/product/cart-actions";
...
function carePills(product: Product) {
  ...
  return [
    ...
    spec.watering || spec.water_schedule
      ? { icon: Droplets, label: String(spec.watering ?? spec.water_schedule) }
      : null,
    spec.difficulty || spec.difficulty_level
      ? { icon: Leaf, label: String(spec.difficulty ?? spec.difficulty_level) }
      : null,
  ...
  return (
    ...
    >
      <Link
        ...
        className="relative block aspect-[1.08/1] overflow-hidden bg-primary-tint"
      >
        {image ? (
          <img
            src={image}
        ...
        <div className="absolute left-2.5 top-2.5 flex flex-wrap gap-1.5">
          {discount > 0 && (
            <span className="rounded-md bg-sale px-2 py-1 text-[10px] font-bold uppercase text-sale-foreground">
              {discount}% off
            </span>
          )}
          {product.is_bestseller && (
            <span className="rounded-md bg-forest px-2 py-1 text-[10px] font-bold uppercase text-forest-foreground">
              Bestseller
            </span>
          )}
          {product.is_new_arrival && (
            <span className="rounded-md bg-primary-soft px-2 py-1 text-[10px] font-bold uppercase text-primary-soft-foreground">
              New
            </span>
          )}
        </div>
      ...
      <div className="flex flex-1 flex-col p-3 sm:p-4">
        ...
        <Link
          to="/product/$id"
          params={{ id: product.id }}
          className="mt-1 line-clamp-2 min-h-10 text-sm font-semibold leading-5 transition-colors duration-200 hover:text-primary"
        >
          {product.title}
        </Link>
        ...
        {pills.length > 0 && (
          <ul className="mt-2.5 flex flex-wrap gap-1.5">
            {pills.map(({ icon: Icon, label }) => (
              <li
                key={label}
                className="flex items-center gap-1 rounded-full bg-primary-tint px-2 py-1 text-[10px] font-medium text-primary-soft-foreground"
              >
        ...
        <div className="mt-3 flex flex-wrap items-baseline gap-x-2 gap-y-1">
          ...
          {mrp && mrp > price && (
            <span className="price-num text-xs font-medium text-muted-foreground line-through">
              {money(mrp)}
            </span>
          )}
        </div>
