...
interface CartContextValue { items: CartItem[]; count: number; subtotal: number; isOpen: boolean; openCart: () => void; closeCart: () => void; setCartOpen: (open: boolean) => void; addItem: (item: CartItem, options?: { openDrawer?: boolean }) => void; updateQty: (productId: string, qty: number, size?: string) => void; removeItem: (productId: string, size?: string) => void; clear: () => void }
...
export function CartProvider({ children }: { children: ReactNode }) {
 ...
 const addItem = useCallback((next: CartItem, options?: { openDrawer?: boolean }) => {
  setItems((current) => { const index = current.findIndex((item) => item.product_id === next.product_id && item.size_variant === next.size_variant && item.pot_type === next.pot_type); if (index < 0) return [...current, next]; return current.map((item, i) => i === index ? { ...item, qty: Math.min(item.qty + next.qty, item.stock ?? 99) } : item); });
 ...
 const value = useMemo(() => ({ items, count: items.reduce((sum, item) => sum + item.qty, 0), subtotal: items.reduce((sum, item) => sum + item.qty * item.unit_price, 0), isOpen, openCart, closeCart, setCartOpen, addItem, updateQty, removeItem, clear }), [items, isOpen, openCart, closeCart, setCartOpen, addItem, updateQty, removeItem, clear]);
